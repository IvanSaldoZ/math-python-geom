...
class TestPolygon:
    ...
    def test_g73_rect_x_lay_trian_up1(self):
        Figure.rectangle = Rectangle(R2Point(0.0, 0.0), R2Point(2.0, 1.0))  # Сразу создаем прямоугольник
        # Сначала делаем отрезок
    ...
    # 13. Функция `g73` вычисляется корректно для лежачего прямоугольника и
    # выпуклой оболочки - треугольника над ним, пересекающего сверху
    # и имеющего общую точку - вершину, которая НЕ должна формировать общую площадь
    # (0.0, 0.0), (2.0, 1.0) - прямоугольник

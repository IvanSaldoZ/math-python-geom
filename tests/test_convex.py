...
class TestPolygon:
    ...
    # 10. Функция `g73` вычисляется корректно для лежачего прямоугольника и
    # выпуклой оболочки - треугольника над ним, пересекающего сверху
    ...
    # 11. Функция `g73` вычисляется корректно для лежачего прямоугольника и
    # выпуклой оболочки - треугольника над ним, пересекающего сверху
    # и имеющего общую точку - вершину, которая НЕ должна формировать общую площадь
    # (0.0, 0.0), (2.0, 1.0) - прямоугольник
    # (0.0, 2.0), (2.0, 0.0), (2.0, 2.0), (0.0, 1.0)
    def test_g73_rect_x_lay_trian_up2(self):
        # Сразу создаем прямоугольник
        Figure.rectangle = Rectangle(R2Point(0.0, 0.0), R2Point(2.0, 1.0))
        t = Point(R2Point(0.0, 2.0))
        t = t.add(R2Point(2.0, 0.0))
        t = t.add(R2Point(2.0, 2.0))
        t = t.add(R2Point(0.0, 1.0))
        # Общая площадь должна быть 1.0 (половина прямоугольника)
        assert t.g73() == approx(1.0)


    # 12. Функция `g73` вычисляется корректно для лежачего прямоугольника и
    # выпуклой оболочки - треугольника над ним, пересекающего сверху
    # и имеющего общую точку - вершину, которая НЕ должна формировать общую площадь
    # (0.0, 0.0), (2.0, 1.0) - прямоугольник
    # (0.0, 2.0), (2.0, 0.0), (2.0, 2.0), (0.0, 1.0)
    ...
    # 13. Функция `g73` вычисляется корректно для треугольника, последнее ребро которого
    ...
    # 14. Функция `g73` вычисляется корректно для выпуклой оболочки
    # четвертое ребро (т.е. Polygon уже создан) которого создается через границу прямоугольника
    ...
    # 15. Функция `g73` вычисляется корректно для выпуклой оболочки
    # Пересечение выпуклой оболочки и прямоугольника идет лишь по границе
    ...
    # 16. Функция `g73` вычисляется корректно для выпуклой оболочки
    # Пересечение выпуклой оболочки и прямоугольника идет лишь по границе
    # Причем новая точка добавляется за отрезком точек, и нужно расширить дек
    # (0.0, 0.0), (2.0, 1.0) - прямоугольник
    ...
    # 17. Функция `g73` вычисляется корректно для выпуклой оболочки
    # Пересечение выпуклой оболочки и прямоугольника идет лишь по границе
    # Причем новая точка добавляется за отрезком точек, и нужно расширить дек
    # (0.0, 0.0), (2.0, 1.0) - прямоугольник
    # (1.0, 1.0), (2.0, 2.0), (0.0, 2.0), (2.0, 1.0), (0.0, 2.0), (0.0, 1.0), (0.0, 2.0), (0.0, 1.0)
